...
to go
  ...
  [
    ...
    ;; bounce against black patches?
    let next-patch patch-at-heading-and-distance heading speed
    if [pcolor] of next-patch = black
    [
      bounce-wall-event
    ]
...
to set-ball-color
  if (ball-color = "red") [ set color red ]
  if (ball-color = "orange") [ set color orange ]
  if (ball-color = "yellow") [ set color yellow ]
  if (ball-color = "lime") [ set color lime ]
  if (ball-color = "green") [ set color green ]
  if (ball-color = "sky") [ set color sky ]
  if (ball-color = "blue") [ set color blue ]
  if (ball-color = "magenta") [ set color magenta ]
  if (ball-color = "violet") [ set color violet ]
  if (ball-color = "pink") [ set color pink ]
end


to set-ball-heading
  if (ball-heading = "up") [ set heading 0 ]
  if (ball-heading = "down") [ set heading 180 ]
  if (ball-heading = "left") [ set heading 270 ]
  if (ball-heading = "right") [ set heading 90 ]
end
...
to bounce-walls
  ...
  let currx [pxcor] of patch-here
  let curry [pycor] of patch-here

  ask patch-at-heading-and-distance heading speed [
    if pcolor = black
    [
      if (pxcor != currx) [ set new-heading (- new-heading) ]
      if (pycor != curry) [ set new-heading (180 - new-heading) ]
    ]
  ]
...
to bounce-balls  ;; ball procedure
  let candidate 0
...
ball-color
ball-color
"red" "orange" "yellow" "lime" "green" "sky" "blue" "magenta" "violet" "pink"
0
