...
to setup
  ...
  create-moose 3  ; create the moose, then initialize their variables
  [
    set shape  "moose"
    set color brown
    set size 7
...
to go
  ...
  if count moose > max-moose [ user-message "The moose have inherited the earth" stop ]

  ask moose [ moose-actions ]

  ask wolves [
    wolf-actions
    let prey one-of moose in-radius 3
    if prey != nobody [
      wolf-meets-moose
    ]
  ]
...
to flee
  ifelse count wolves in-radius 4 > 0 [ set heading towards one-of wolves in-radius 2
  rt 180
  ][
    rt random 50
    lt random 50
...
"moose" 1.0 0 -4699768 true "" "plot count moose"
